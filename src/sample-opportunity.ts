...
const main = async () => {
  ...
  const challengeID = 1;
  const response = await populator.createOpportuntiy(challengeID, opportunityJson.name, opportunityJson.textID);
  populator.logger.verbose(`Created opportunity with name: ${response.createOpportunityOnChallenge.name}`);
  const opportunityID = response.createOpportunityOnChallenge.id;

  // Create actor groups
  const stakeholderAG = await populator.createActorGroup(opportunityID, 'stakeholders', "test");
  const stakeholders = opportunityJson.stakeholders;
  for (let i = 0; i < stakeholders.length; i++) {
    const stakeholder = stakeholders[i];
    const stakeholderResponse = await populator.createActor(stakeholderAG.createActorGroup.id, stakeholder.name, stakeholder.wins_how);
    populator.logger.verbose(`${stakeholderResponse}`);
  }
  const keyUsersAG = await populator.createActorGroup(opportunityID, 'key_users', "test");
  const keyUsers = opportunityJson.key_users;
  for (let i = 0; i < keyUsers.length; i++) {
    const keyUser = keyUsers[i];
    const keyUserResponse = await populator.createActor(keyUsersAG.createActorGroup.id, keyUser.name, keyUser.wins_how);
    populator.logger.verbose(`${keyUserResponse}`);
  }
  const collaboratorsAG = await populator.createActorGroup(opportunityID, 'collaborations', "test");
  ...
  for (let i = 0; i < solutionAspectNames.length; i++) {
    ...
    const aspectResponse = await populator.createAspect(opportunityID, name, solution.question, solution.explanation);
    populator.logger.verbose(`${aspectResponse.createAspect.title}`);
  }
  


  populator.logger.verbose(`Finished creating opportunity: ${response.createOpportunityOnChallenge.name}`);
